...
while read flocation; do if test -e ${flocation} && test "$( $flocation --fcheck 2>/dev/null )" -ge 20170111; then frameworkscript=$flocation; break; fi; done <<EOFLOCATIONS
./framework.sh
