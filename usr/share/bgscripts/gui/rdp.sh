...
while read flocation; do if test -e $flocation && test $( $flocation --fcheck 2>/dev/null -ge 20151123; then frameworkscript=$flocation; break; fi; done <<EOFLOCATIONS
./framework.sh
...
userfile=~/.config/bgscripts/.bgstack15.ipa.smith122.com # is configurable with -U flag
